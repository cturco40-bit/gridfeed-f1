...
export default async (req, context) => {
  ...
  try {
    ...
    console.log('[fetch-locations] session_key:', session.session_key, 'meetingLabel:', JSON.stringify(meetingLabel), 'race_id:', raceId);
    if (!raceId) {
      console.warn('[fetch-locations] matchRaceId returned null — car_locations rows will have race_id=null');
    }
    ...
    console.log('[fetch-locations] OpenF1 GET:', path);
    const res = await fetchOpenF1(path, 20000);
    if (!res.ok) throw new Error(`OpenF1 location HTTP ${res.status}`);
    const locs = await res.json();
    console.log('[fetch-locations] OpenF1 rows:', Array.isArray(locs) ? locs.length : 'not-array');
    if (!Array.isArray(locs) || !locs.length) {
      await logSync('fetch-locations', 'success', 0, `No location data in window (${session.session_name}) since=${since}`, Date.now() - start);
      return json({ ok: true, totalRecords: 0, reason: 'no_data' });
    }

    // OpenF1 returns null x/y for cars parked in the pits between laps
    const filteredOut = locs.length;
    let valid = locs.filter(l => Number.isFinite(l.x) && Number.isFinite(l.y) && l.driver_number != null && l.date);
    console.log('[fetch-locations] valid after null-filter:', valid.length, '/', filteredOut);
    ...
    console.log('[fetch-locations] valid after dedup (lastDate=' + (lastDate || 'none') + '):', valid.length);

    if (!valid.length) {
      await logSync('fetch-locations', 'success', 0, `Up to date (${session.session_name}, lastDate=${lastDate || 'none'}, openf1=${filteredOut})`, Date.now() - start);
      return json({ ok: true, totalRecords: 0, reason: 'no_new' });
    }

    // OpenF1 fields → car_locations columns: session_key, driver_number,
    // date, x, y, z map 1:1. race_id comes from matchRaceId(meetingLabel).
    ...
    const CHUNK = 1000;
    let chunksOk = 0, chunksFailed = 0;
    for (let i = 0; i < rows.length; i += CHUNK) {
      const slice = rows.slice(i, i + CHUNK);
      const result = await sb('car_locations', 'POST', slice);
      // sb() returns the inserted body on success, or `false`/empty on failure.
      const okay = Array.isArray(result) ? true : (result === true || (result && typeof result === 'object'));
      if (okay) {
        chunksOk++;
        totalRecords += slice.length;
      } else {
        chunksFailed++;
        console.error('[fetch-locations] sb() insert returned falsy for chunk', i, 'size', slice.length, 'sample row:', JSON.stringify(slice[0]));
      }
    }
    if (chunksFailed) {
      throw new Error(`Supabase insert failed on ${chunksFailed}/${chunksOk + chunksFailed} chunks — see function logs for sample row`);
    }
